...
usage() {
    cat <<'EOF'
Usage: ./test-intel-browser.sh --capture-mode compat|direct [options]

The Intel server must already be running on nzxt5.
Options:
  ...
  --render-node NODE   Expected node, e.g. D130 or /dev/dri/renderD130
  --help               Show this help
EOF
}
...
render_node="${LLRDC_RENDER_NODE:-D130}"
...
case "$render_node" in
    /dev/dri/render*) expected_render_node="$render_node" ;;
    D*) expected_render_node="/dev/dri/render${render_node}" ;;
    *) echo "--render-node must be Dxxx or /dev/dri/renderDxxx." >&2; exit 2 ;;
esac

require_playwright_runtime
configure_installed_chrome
export LLRDC_ACCELERATOR=intel
...
export LLRDC_EXPECTED_RENDER_NODE="$expected_render_node"

exec npx playwright test tests/intel-browser/gpu_connection.spec.ts --workers=1 --reporter=line --timeout=60000
