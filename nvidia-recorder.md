...
* **Update:** NVENC initialization has since been moved onto the explicit preset-config path and no longer hardcodes `30 FPS`; the direct helper now receives the requested `--fps` and `--bitrate`, and Linux framerate changes also reapply the Wayland headless output mode so the compositor refresh rate tracks runtime `30 -> 60` changes.
* **Current Outcome:** On real NVIDIA hardware, the direct path now starts with `--fps 60`, the native helper logs `Requested target FPS: 60`, and `wlr-randr` reports `1920x1080 px, 60.000000 Hz (current)` for `HEADLESS-1`.
