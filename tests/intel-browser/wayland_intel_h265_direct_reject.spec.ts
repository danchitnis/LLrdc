...
test.describe('Wayland Intel H.265 Direct Rejection', () => {
    test('should reject Intel h265_vaapi in direct-buffer mode instead of starting a dead stream', async () => {
        ...
        expect(output).toContain('Invalid direct-buffer configuration: Intel H.265 hardware encode is not supported on this FFmpeg/driver stack; use h264_vaapi or av1_vaapi for direct mode');
    });
});
